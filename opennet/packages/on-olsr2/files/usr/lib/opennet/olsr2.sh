#jshn needed to handle json content, e.g. json_load,json_select,...
...
debug_fetch_ipv4_from_ipv6_for_ap() {
	...
	if [ "$mac" != "" ]; then
		json=$(wget -q -O - "https://api.opennet-initiative.de/api/v1/interface/?if_hwaddress=$mac")
		#delete '[' and ']' because else json_load will fail, see also http://eko.one.pl/forum/viewtopic.php?pid=157702#p157702
		json="${json//[/}"
		json="${json//]/}"
...
debug_fetch_ipv4_from_ipv6_for_ap fd32:d8d3:87da::15:6dff:fe80:97b
#debug_fetch_ipv4_from_ipv6 fd32:d8d3:87da::1298:36ff:feb2:5271
